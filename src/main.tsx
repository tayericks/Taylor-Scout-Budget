...
function App() {
  ...
  return (
    <div className="app-shell">
      ...
      {activeModal === 'connections' && <ConnectionsModal budget={budget} activeShow={activeShow} onClose={()=>setActiveModal(null)} onImport={(records)=>{ const next=[...budgets]; records.forEach((r:any)=>{ const existing=next.find(b=>b.sharedLocationId===r.sharedLocationId&&b.episode===r.episode&&b.setName===r.setName); if(existing) Object.assign(existing,r); else next.push({...r,id:crypto.randomUUID(),showId:activeShow.id,production:activeShow.name,version:'Budget V1',cityId:activeShow.defaultCityId||'la-city',contingency:activeShow.defaultContingency,keyAssistantLocationManager:r.keyAssistantLocationManager||'',items:templateItems(),customSections:[],sectionOverrides:{}}) }); setBudgets(next); const first=next.find(b=>b.showId===activeShow.id&&records.some((r:any)=>r.sharedLocationId===b.sharedLocationId)); if(first)setActiveBudgetId(first.id); setActiveModal(null) }} />}
...
function ConnectionsModal({budget,activeShow,onClose,onImport}:{budget:BudgetPage,activeShow:ShowProfile,onClose:()=>void,onImport:(records:any[])=>void}) {
  const [message,setMessage]=useState('')
  const importFile=(file?:File)=>{ if(!file)return; const reader=new FileReader(); reader.onload=()=>{ try{ const raw=JSON.parse(String(reader.result||'{}')); const events=Array.isArray(raw)?raw:(raw.locations||raw.events||[]); const records=events.filter((e:any)=>e.eventType!=='note').map((e:any)=>({episode:String(e.episode||'Episode'),setName:e.set||e.setName||'New Set',setNumber:e.scenes||'',scenes:e.scenes||'',location:e.location||'',address:e.address||'',contact:e.contact||'',phone:e.phone||'',keyAssistantLocationManager:Array.isArray(e.keyIds)?e.keyIds.join(', '):(e.key||''),sharedLocationId:e.sharedLocationId||String(e.location||'').toLowerCase().replace(/[^a-z0-9]+/g,'-'),calendarAssignmentIds:[e.id].filter(Boolean),prepStart:e.prepStart,prepEnd:e.prepEnd,shootStart:e.shootStart,shootEnd:e.shootEnd,holdStart:e.holdStart,holdEnd:e.holdEnd,strikeStart:e.strikeStart,strikeEnd:e.strikeEnd})); if(!records.length)throw new Error('No assignments found'); onImport(records) }catch(err){setMessage('Could not read that Calendar export. Use the JSON Backup or Episode Budget Package.') } }; reader.readAsText(file) }
  const biblePackage={version:1,show:{id:activeShow.id,name:activeShow.name,season:activeShow.season},budget:{id:budget.id,episode:budget.episode,setName:budget.setName,scenes:budget.scenes||budget.setNumber,location:budget.location,address:budget.address,contact:budget.contact,phone:budget.phone,keyAssistantLocationManager:budget.keyAssistantLocationManager,sharedLocationId:budget.sharedLocationId,dates:{prep:[budget.prepStart,budget.prepEnd],shoot:[budget.shootStart,budget.shootEnd],hold:[budget.holdStart,budget.holdEnd],strike:[budget.strikeStart,budget.strikeEnd]},approvedBudget:budget.items.reduce((sum,i)=>sum+calcItem(i),0),poNumbers:budget.items.filter(i=>i.poNumber).map(i=>({description:i.name,poNumber:i.poNumber,vendor:i.vendor||''})),vendors:budget.items.filter(i=>i.vendor).map(i=>({vendor:i.vendor,description:i.name,status:i.status||'estimate',budgetAmount:calcItem(i),poNumber:i.poNumber||''}))}}
  return <div className="modal-backdrop"><div className="modal connections-modal"><div className="modal-head"><div><span className="eyebrow">SHARED SHOW DATA</span><h2>Calendar / Budget / Bible</h2></div><button className="icon-btn" onClick={onClose}><X/></button></div><p className="library-help">Calendar is the source for episode, set, scenes, physical location, dates and assigned key. Budget adds estimated costs and PO numbers. Bible receives the locked budget and sends ordered commitments back later.</p><div className="connection-grid"><section><CalendarDays size={25}/><h3>Import Calendar</h3><p>Create or update budget pages from a Calendar JSON export.</p><label className="primary upload-connection"><Upload size={16}/> Select Calendar JSON<input type="file" accept="application/json,.json" onChange={e=>importFile(e.target.files?.[0])}/></label></section><section><BookOpen size={25}/><h3>Create Bible Package</h3><p>Exports scene, location, dates, contacts, vendors, PO numbers and the locked approved budget.</p><button className="primary" onClick={()=>downloadJson(`${activeShow.name}_${budget.episode}_${budget.location}_Bible.json`,biblePackage)}><Download size={16}/> Export to Bible</button></section><section><Download size={25}/><h3>Budget Backup</h3><p>Download the complete editable budget page before deployment or migration.</p><button className="secondary" onClick={()=>downloadJson(`${activeShow.name}_${budget.episode}_${budget.location}_Budget.json`,budget)}><Download size={16}/> Export Budget</button></section></div>{message&&<div className="delete-warning">{message}</div>}<div className="connection-status"><strong>Connected record</strong><span>{budget.episode} · {budget.setName}</span><span>{budget.location||'No physical location'}</span><small>{budget.sharedLocationId?'Shared location ID: '+budget.sharedLocationId:'Not yet linked to Calendar'}</small></div><div className="modal-actions"><button className="secondary" onClick={()=>window.location.href='https://calendar.taylorscout.com'}>Open Calendar</button><button className="secondary" onClick={()=>window.location.href='https://bible.taylorscout.com'}>Open Bible</button><button className="primary" onClick={onClose}>Done</button></div></div></div>
}
