...
export async function saveBudgetDocument(showId:string,payload:any){
  ...
  for(const budget of budgets){
    ...
    if(current&&same(cleanBudget(remoteBudget),local)){budgetTokens.set(tokenFor(showId,locationId),current.updated_at);budget.__remoteUpdatedAt=current.updated_at;latest=current.updated_at||latest;continue}
    if(current&&known&&current.updated_at!==known){conflicts.push(locationId);continue}
    if(current&&!known){conflicts.push(locationId);continue}
