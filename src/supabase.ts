...
export async function saveBudgetDocument(showId:string,payload:any){
  ...
  const presentLocations=new Set<string>();const known=loadedBudgetIds.get(showId)||new Map<string,string>();const tombstonesAtLoad=loadedBudgetTombstones.get(showId)||new Set<string>();const seenLocationIds=new Map<string,string>()
  for(const budget of budgets){
    if(budget.sharedLocationId){const priorBudgetId=seenLocationIds.get(budget.sharedLocationId);if(priorBudgetId&&priorBudgetId!==budget.id){budget.sharedLocationId=undefined}}
    const locationId=await ensureLocationId(showId,budget);seenLocationIds.set(locationId,budget.id);presentLocations.add(locationId);let state=await lifecycleState(showId,locationId);if(state.locationDeleted||!state.location||state.location.metadata?.archived_at){conflicts.push(locationId);continue}if(state.budgetDeleted){if(!tombstonesAtLoad.has(locationId)){conflicts.push(locationId);continue}const{error:clearError}=await supabase.from('tool_documents').delete().eq('show_id',showId).eq('tool_key',budgetTombstoneKey(locationId));if(clearError)throw clearError;tombstonesAtLoad.delete(locationId);state={...state,budgetDeleted:false}}
